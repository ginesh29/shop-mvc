//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace SPA
{
    using System;
    using System.Collections.Generic;
    
    public partial class SPA_INVOICE_Detail
    {
        public long Invoice_Detail_Id { get; set; }
        public long Invoice_Id { get; set; }
        public Nullable<int> ReservationId { get; set; }
        public Nullable<double> Quantity { get; set; }
        public Nullable<long> Add_On_Pid { get; set; }
        public string ProductName { get; set; }
        public Nullable<double> FiveMinuteChargeorPercharge { get; set; }
        public Nullable<double> TotalPrice { get; set; }
        public Nullable<int> Duration { get; set; }
        public string Manufacturer { get; set; }
        public string Dozes { get; set; }
        public long SchlId { get; set; }
        public Nullable<long> Product_Id { get; set; }
        public Nullable<System.DateTime> Date_Of_Buying { get; set; }
        public string ActiveStatus { get; set; }
        public System.DateTime CreatedDate { get; set; }
        public System.DateTime UpdatedDate { get; set; }
        public Nullable<long> Field1 { get; set; }
        public Nullable<long> field2 { get; set; }
        public string field3 { get; set; }
        public string field4 { get; set; }
        public Nullable<System.DateTime> Field5 { get; set; }
        public Nullable<double> VAT { get; set; }
        public Nullable<double> TotalPricewithvat { get; set; }
        public Nullable<long> Health_Assign_id { get; set; }
        public string Settlement_text { get; set; }
        public Nullable<int> Settlement_Number { get; set; }
        public Nullable<int> Tarif_Number { get; set; }
    
        public virtual Add_on_Product_Master Add_on_Product_Master { get; set; }
        public virtual CCTSP_CategoryDetails CCTSP_CategoryDetails { get; set; }
        public virtual CCTSP_SchoolMaster CCTSP_SchoolMaster { get; set; }
        public virtual SPA_EmployeeScheduler SPA_EmployeeScheduler { get; set; }
        public virtual SPA_INVOICE_MASTER SPA_INVOICE_MASTER { get; set; }
    }
}
